        ...
        items=[
            ui.inline(
                items=[
                    ...
                    ui.button(name="show_add_location_panel", label="Add", icon="add"),
                ]
            ),
            ui.table(
                name="table",
...
async def show_add_location_panel(q):
    ...
    items = [
        ui.dropdown(
            name="location_type",
            value=str(q.client.location_type),
            choices=[
                ui.Choice(
                    name=str(db.LocationType.Local.value),
                    label=db.LocationType.Local.name,
                ),
                ui.Choice(
                    name=str(db.LocationType.Agent.value),
                    label=db.LocationType.Agent.name,
                ),
            ],
            label="Type",
            required=True,
    ...
    if q.client.location_type == str(db.LocationType.Local.value):
        items += [
            ui.textbox(
                name="location_name",
                label="Name",
                value=q.client.location_name,
                required=True,
            ),
            ui.textbox(
                name="location_path",
                label="Path",
                value=q.client.location_path,
                required=True,
        ...
        items += [
            ...
            ui.textbox(
                name="location_uri",
                label="Remote Agent ip addr with port",
                value=q.client.location_uri,
                required=True,
                placeholder="Remote agent address (e.g. 127.0.0.1:11011)",
            ),
            ui.textbox(
                name="location_path",
                label="Path",
                value=q.client.location_path,
                required=True,
                placeholder="Path on the remote",
            ),
            ui.button(name="prescan_location", label="Scan"),
            ui.button(name="add_location", label="Add"),
        ]
...
def update_location_panel(q, loc):
    ...
    q.page["meta"].side_panel = ui.side_panel(
        ...
        items=[
            ...
            ui.textbox(
                name="location_uri", label="Uri", value=str(loc.uri), required=False
            ),
