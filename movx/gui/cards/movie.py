...
def add_movie_cards(q, movie):
    ...
    q.page.add(
        "movie_header", ui.form_card(
        box=ui.box("infobar"),
        items=[
            ui.inline(
                items=[
                    ui.text_xl("Movie %s" % movie.title),
                    ui.tags([ ui.tag(color=t.color, label=t.name) for t in movie.tags ]),
                    ui.picker(name='movie_tags_picker', label='Tags', values=movie_tags, choices=[
...
def dcps_table(dcps):
    ...
    for dcp in dcps:
        ...
        tags = [ '<span class="wave-w6 wave-s12" style="background-color:%s; border-radius: 4px; padding: 4px 8px;">%s</span>' % (tag.color, tag.name) for tag in dcp.tags ]
        rows.append([
