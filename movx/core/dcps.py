...
from movx.core.locations import Location
...
def parse_task(dcp, probe=False):
    ...
    dcp.update(package_type = report.get("package_type", "??"),
...
def update_movie(dcp, movie_title):
    with session:
        movie = session.scalars( select(Movie).filter(Movie.title == movie_title)).first()
