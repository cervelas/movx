...
def generate_tasks_rows(tasks):
    rows = []
    for task in tasks:
        rows.append(ui.table_row(name="#task/%s" % task.id, cells=[task.name, task.type, task.status,
...
def tasks_list(tasks):
    ...
    columns = [
        ui.table_column(name='name', label='Name', searchable=True, min_width="600px", cell_overflow='wrap'),
        ui.table_column(name='type', label='Type', searchable=True, min_width="40px"),
        ui.table_column(name='status', label='status', filterable=True, max_width="150px"),
        ui.table_column(name='started', label='started', max_width="200px", data_type='time'),
        ui.table_column(name='time_elapsed', label='duration', max_width="70px"),
        ui.table_column(name='progress', label='progress', cell_type=ui.progress_table_cell_type(), max_width="70px"),
    ]
...
def check_task_details(task):
    ...
    columns = [
        ...
        ui.table_column(name='errors', label='progress', filterable=True, cell_type=ui.progress_table_cell_type(), max_width="200px"),
        ui.table_column(name='time_elapsed', label='duration', max_width="40px"),
        ui.table_column(name='result', label='result', searchable=True, filterable=True, max_width="500px"),
    ]

    return ui.form_card(box=ui.box('header', size=0),
            items=[
...
def task_details(task):
    ...
    return [
            ui.inline(justify="between", items = [
                    ...
                    ui.text_s("Finished @ %s" % time.ctime(task.timestamp + task.elapsed_time_s)),
                    ui.text_s("Duration %s" % timedelta(seconds=round(task.elapsed_time_s)))
                ] ),
