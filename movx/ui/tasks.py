...
async def poll_tasks(q: Q):
    ...
    while True:
        ...
        q.page["tasks_list"].table.rows = generate_tasks_rows(tasks)
...
async def task_detail_layout(q: Q, id):
    ...
    if task:
        ...
        q.page["task_detail_%s" % task.id] = ui.form_card(box=ui.box('content', size=0), items=[
                                    ui.inline(justify="between", items = [ 
                                        ui.button(name='#alltasks', label='Back', icon="ChevronLeftMed"),
                                        ui.text_xl("Task %s : %s" % (task.name, task.status)), 
                                        ui.button(name='#dcp/%s' % task.dcp.id, label='%s' % task.dcp.title, icon="VideoSearch"),
                                    ]),
        ] +  task_details(task))
    else:
        setup_page(q, "Error")
        q.page["not-found"] = ui.form_card(box="content", items=[
            ui.text("Task %s not found" % id)
        ])
...
async def tasks_layout(q: Q):
    ...
    if len(ts) > 0:
        q.page["tasks_list"] = ui.form_card(box="content", items=[
            ui.inline(items = [ ui.text_xl("Tasks"), 
                                ui.button(name='refresh_tasks', label='', icon="refresh"),
                                ui.button(name="delete_all_tasks", label="Clear All", icon="danger"),
                                ]),
            tasks_list(ts)
        ])
    else:
