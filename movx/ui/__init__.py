...
def breadcrumbs(q: Q, crumbs=[]):
    ...
    if len(crumbs) > 0:
        q.page["breadcrumbs"] = ui.breadcrumbs_card(box='content', items=[ ui.breadcrumb(name=b[0], label=b[1]) for b in crumbs])
    ...
    return ui.form_card(
        ...
        items=[
            ...
            ui.expander(name='report', label='Error Details', items=[
                ui.text(
                    f'To report this issue, <a href="" target="_blank">please open an issue</a> with the details below:'),
                ui.text_l(content=f'Report Issue in App: **MovX**'),
                ui.text(content='\n'.join(dump)),
            ])
        ]
    )
...
def dict_to_searchable_table(dic):
    ...
    return ui.table(name='table', height='600px', downloadable=True, resettable=True, columns=[
            ui.table_column(name='name', label='Name', searchable=True, min_width="600px", cell_overflow="wrap", align='right'),
            ui.table_column(name='value', label='Value', searchable=True, min_width="600px", cell_overflow="wrap"),
        ], rows=[
            ui.table_row(name=f'row.{k}', cells=[str(k), str(v)]) for k, v in dic.items()
        ])
